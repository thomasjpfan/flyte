...
func applyExtendedResourcesOverrides(base, overrides *core.ExtendedResources) *core.ExtendedResources {
	...
	if overrides.GetSharedMemoryVolume() != nil {
		new.SharedMemoryVolume = overrides.GetSharedMemoryVolume()
	}

	return new
}

func ApplySharedMemoryVolume(podSpec *v1.PodSpec, primaryContainerName string, sharedMemoryVolume *core.SharedMemoryVolume) error {
	sharedMountName := sharedMemoryVolume.GetMountName()
	sharedMountPath := sharedMemoryVolume.GetMountPath()
	if sharedMountName == "" {
	...
	var err error
	if sharedMemoryVolume.GetSizeLimit() != "" {
		quantity, err = resource.ParseQuantity(sharedMemoryVolume.GetSizeLimit())
		if err != nil {
...
func ApplyFlytePodConfiguration(ctx context.Context, tCtx pluginsCore.TaskExecutionContext, podSpec *v1.PodSpec, objectMeta *metav1.ObjectMeta, primaryContainerName string) (*v1.PodSpec, *metav1.ObjectMeta, error) {
	...
	if extendedResources.GetSharedMemoryVolume() != nil {
		err = ApplySharedMemoryVolume(podSpec, primaryContainerName, extendedResources.GetSharedMemoryVolume())
		if err != nil {
			return nil, nil, err
		}
	}
